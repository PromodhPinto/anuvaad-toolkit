...
from config import  PROCESS,OUTPATH,params
import time
import json
import sys
import os
...
def train_chunk():
    while True:
        ...
        create_image(ls[0],ls[1],ls[2],ls[3],ls[4],ls[5])
...
if __name__ == '__main__':
    ...
    while True :
        ...
        while train_queue.qsize() < PROCESS :
            try:
                background,font,symbol,font_size,col,font_colour = para.__next__()
                ls=[background,font,symbol,font_size,col,font_colour]
                train_queue.put(ls)
                #time.sleep(.1)
    ...
    with open(OUTPATH+"data/"+"params"+".json", 'w') as outfile:
